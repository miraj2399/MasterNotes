...
const RemoveDiscussionPostHandler = async (req, res) => {
    ...
    try{
        ...
        const deletedDiscussionPost = await DiscussionPost.findByIdAndDelete(req.params.id);
...
const RemoveCommentHandler = async (req, res) => {
    ...
    try{
        ...
        const discussionPost = await DiscussionPost.findById(comment.discussionPost);
        if (!discussionPost) {
            return res.status(404).json({
                message: "Discussion post not found",
            });
